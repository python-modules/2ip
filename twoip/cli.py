...
def __key(key: str = None, keyfile: TextIO = None) -> Union[str, None]:
    ...
    if keyfile:
        ...
        if key:
            log.warn(
                "Both an API key and API key file have been specified; the API key file takes priority"
            )
        ## Attempt key load
        try:
            ...
            if key:
                log.warn(
                    "Could not load API key from file; using API key option from command line instead"
                )
            else:
                log.warn(
                    "Could not load API key from file; defaulting to not using any API key - rate limits will apply"
                )
        else:
...
def __load_key(file: TextIO) -> str:
    ...
    if key:
        ...
        log.warn(
            f'No API key could be retrieved from file "{file.name}"; maybe it is empty'
        )
...
def cli(
    ...
    if connections < 1 or connections > 100:
        log.fatal(
            f"A minimum of 1 connection and a maximum of 100 connections must be used"
        )
