...
This file provides the logging functionality for the TwoIP API Client.
"""
...
class Log(object):
    ...
    This logging handler will use the python "logging" module to output logs.
    If colorlog is available, the logs will be colored.

    For a slight performance boost, calls to log verbose, debug or trace level
    events will be skipped when running Python with the Optimize flag (-O).
    ...
    def __init__(self, verbosity: int = def_verbosity) -> object:
        ...
        Args:
            verbosity (int, optional): The verbosity level. Defaults to
            logging.INFO.
        ...
        ## Setup logging
        self.__setup(colorlog=colorlog)
    ...
    def __setup(colorlog: bool) -> None:
        ...
        ## A check is done for __debug__ (to determin if running in optimized
        ## mode); if not, a custom logging format is used
        if __debug__:

            ## Use the CustomLogFactory function to fix up spacing of logged
            ## messages
